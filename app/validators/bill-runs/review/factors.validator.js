...
function go(payload) {
  const schema = Joi.object({
    amendedAggregate: Joi.number().min(0).required().custom(_maxDecimals, 'Max decimals').messages({
      ...
      'number.unsafe': 'The aggregate factor must be greater than 0 and not exceed 9007199254740991',
    ...
    amendedChargeAdjustment: Joi.number().min(0).required().custom(_maxDecimals, 'Max decimals').messages({
      ...
      'number.unsafe': 'The charge factor must be greater than 0 and not exceed 9007199254740991',
