 ...
 * Handles the user submission for the `/data/deduplicate` page
 *
 * It will first validate that the user has entered a reference. If they haven't we return an error that can be used
 * by the Nunjucks view to let the user know.
 *
 * If they have we first parse it, removing any whitespace and converting any lowercase characters to uppercase. This
 * parsed reference is then pass to the `DeDuplicateService` to do the actual removal of the duplicate licence.
 *
 ...
 *
 * @returns {Promise<Object>} nothing if the user submitted a value else an object containing an error message
 */
