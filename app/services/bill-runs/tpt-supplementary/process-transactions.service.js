...
/**
 * Fetches matching debit transactions from previous bill runs, then compares them as credits those just generated
 ...
 * Fetches matching debit transactions from previous bill runs, then compares them as credits those just generated
 *
 * The `FetchPreviousTransactionsService` finds all previous debit and credit transactions. It then compares the credits
 * to the debits. If they match, the debit transaction is dropped. This is because it must have been dealt with by a
 * previous supplementary bill run.
 *
 ...
 * where we can.
 *
 * Any generated transactions and reversed debits (now credits) that didn't match are returned as the transactions to
 * be sent to the Charging Module API and to be used in the bill.
 *
...
/**
 * Compares a generated transaction to the transactions from previous bill runs
 *
 * Takes a single generated debit transaction and checks to see if the provided array of reversed (credit) transactions
 ...
 * cancel the generated transaction; in this case, we remove the reversed transaction from the array as it can only
 * cancel one generated transaction.
 *
 * @private
 */
function _cancelGeneratedTransaction(generatedTransaction, reversedTransactions) {
  const result = reversedTransactions.findIndex((reversedTransaction) => {
    return transactionsMatch(reversedTransaction, generatedTransaction)
  })
 ...
 * transactions (both generated transactions and reverse transactions) are returned.
 *
 * @private
 */
function _cleanseTransactions(generatedTransactions, reverseTransactions) {
  const cleansedTransactionLines = []

  // Iterate over each generated transaction to see if a transaction in the reverse transactions would form a
  // "cancelling pair" with it. If not then add the unpaired generated transaction to our array of cleansed transaction
  ...
  generatedTransactions.forEach((generatedTransaction) => {
    if (!_cancelGeneratedTransaction(generatedTransaction, reverseTransactions)) {
      cleansedTransactionLines.push(generatedTransaction)
    }
  })
