...
async function go (billRunId, licenceId, status) {
  const { returnLogs, licence, billRun } = await FetchReviewLicenceResultsService.go(billRunId, licenceId)

  const { matchedReturns, unmatchedReturns, chargePeriods } = await PrepareReviewLicenceResultsService.go(returnLogs)

  const pageData = await ReviewLicencePresenter.go(matchedReturns, unmatchedReturns, chargePeriods, licence, billRun, status)
