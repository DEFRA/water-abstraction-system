 ...
 *
 * The charge period is determined as the overlap between the charge information and licence's start and end dates, and
 * the billing period. So, to determine the charge period we look for
 *
 * - the latest start date amongst the charge information, licence and billing period
 * - the earliest end date amongst the charge information, licence and billing period
 *
 * For example, if the licence started on 2017-10-14, the charge information 2022-04-01 and the current billing period
 * is 2023-24 then 2023-04-01 would be the start date.
 *
 ...
 * but the 2023-24 debit for it needs to be credited. We only know to do that by fetching it when running the
 * `ProcessBillingPeriodService` for 2023-24 so `ProcessTransactionsService` will fetch the previous transactions for
 * it.
 *
 * Add to that licences that end before the charge information or billing period and you get scenarios where the start
 * and end date are incompatible
 *
 * ### An out of period charge information
 *
 * - start dates: charge information 2022-04-01, Licence 2017-10-14, Billing period 2023-04-01
 * - end dates: charge information 2022-08-01, Licence 2028-05-01, Billing period 2024-03-31
 * - result: **incompatible!** start date 2023-04-01 to end date 2022-08-01
 *
 * ### A revoked licence
 *
 * - start dates: charge information 2022-04-01, Licence 2017-10-14, Billing period 2023-04-01
 * - end dates: charge information not set, Licence 2023-01-11, Billing period 2024-03-31
 * - result: **incompatible!** start date 2023-04-01 to end date 2023-01-11
 *
 * ### An expired licence
 *
 * - start dates: charge information 2023-10-01, Licence 2017-10-14, Billing period 2023-04-01
 * - end dates: charge information not set, Licence 2023-08-11, Billing period 2024-03-31
 ...
 *
 * @param {module:ChargeInformationModel} chargeInformation The charge information being processed for billing
 * @param {Object} billingPeriod Object with a `startDate` and `endDate` property representing the period being billed
 *
 * @returns {Object} The start and end date of the calculated charge period
 */
function go (chargeInformation, billingPeriod) {
  const latestStartDateTimestamp = Math.max(
    billingPeriod.startDate,
    chargeInformation.startDate,
    chargeInformation.licence.startDate
  )

  // We use .filter() to remove any null timestamps, as Math.min() assumes a value of `0` for these
  const endDateTimestamps = [
    billingPeriod.endDate,
    chargeInformation.endDate,
    chargeInformation.licence.expiredDate,
    chargeInformation.licence.lapsedDate,
    chargeInformation.licence.revokedDate
