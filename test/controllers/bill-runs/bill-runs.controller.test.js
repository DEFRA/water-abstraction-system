...
describe('Bill Runs controller', () => {
  ...
  describe('POST /bill-runs', () => {
    describe('when a request is valid', () => {
      ...
      beforeEach(async () => {
        Sinon.stub(ProcessBillingBatchService, 'go').resolves(validResponse)
      })

      it('returns a 200 response including details of the new billing batch', async () => {
        const response = await server.inject(options())
        console.log('🚀 ~ file: bill-runs.controller.test.js:66 ~ it ~ response:', response)
        const payload = JSON.parse(response.payload)
        console.log('🚀 ~ file: bill-runs.controller.test.js:68 ~ it ~ payload:', payload)
    ...
    describe('when the request fails', () => {
      ...
      describe('because the billing batch could not be initiated', () => {
        beforeEach(async () => {
          ...
          Sinon.stub(ProcessBillingBatchService, 'go').rejects()
        })
