...
describe.only('Notices - Fetch returns address service', () => {
  let seedData
  let session
  ...
  describe('when the licence is registered ', () => {
    ...
    describe.only('and there is a "primary user"', () => {
      beforeEach(async () => {})

      it.only('returns the "primary user" ', async () => {
        console.log(seedData.primaryUser.licenceRef)
        const result = await FetchReturnsAddressService.go([seedData.primaryUser.licenceRef], {
