...
describe('Notices - Fetch Failed Returns Invitations service', () => {
  ...
  describe('when there is a matching notice with multiple licences and return logs', () => {
    before(async () => {
      event = await EventHelper.add({
        ...
        licences: ['11/111', '01/124', '01/125', '12/222', '33/333'],
        metadata: {
  ...
  describe('when there is a matching notice with one licence and return log', () => {
    before(async () => {
      event = await EventHelper.add({
        issuer: 'admin-internal@wrls.gov.uk',
        licences: ['11/111'],
      ...
      await NotificationHelper.add({
        ...
        plaintext: 'Dear Clean Water Limited,\r\n',
        recipient: 'error@example.com',
        returnLogIds: ['18998ffd-feaf-4e24-b998-7e7af026ba14'],
        status: 'error'
      })
    })
    ...
    it('returns the unique list of licence references that have an error with no alternateNoticeId', async () => {
      ...
      expect(result).to.equal({
        failedLicenceRefs: ['11/111'],
        failedReturnIds: ['18998ffd-feaf-4e24-b998-7e7af026ba14']
      })
    })
  })
