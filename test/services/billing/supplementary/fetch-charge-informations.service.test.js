...
describe('Fetch Charge Informations service', () => {
  ...
  describe('when there are charge informations that should be considered for the next supplementary billing', () => {
    ...
    beforeEach(async () => {
      ...
      // This creates a 'current' SROC charge information which covers only FYE 2024
      const sroc2024ChargeInformation = await ChargeInformationHelper.add(
        { startDate: new Date('2023-11-01'), changeReasonId: changeReason.changeReasonId, licenceId }
      )

      // This creates a 'current' SROC charge information which covers both FYE 2023 and 2024
      const sroc2023And24ChargeInformation = await ChargeInformationHelper.add(
        { endDate: new Date('2023-10-31'), changeReasonId: changeReason.changeReasonId, licenceId }
      )

      // This creates a 'current' SROC charge information which covers only FYE 2023
      const sroc2023ChargeInformation = await ChargeInformationHelper.add(
        { endDate: new Date('2022-10-31'), changeReasonId: changeReason.changeReasonId, licenceId }
      )

      // This creates a 'superseded' SROC charge information
      const srocSupersededChargeInformation = await ChargeInformationHelper.add(
        { changeReasonId: changeReason.changeReasonId, status: 'superseded', licenceId }
      )

      // This creates an ALCS (presroc) charge information
      const alcsChargeInformation = await ChargeInformationHelper.add(
        { scheme: 'alcs', licenceId }
      )

      testRecords = [
        sroc2024ChargeInformation,
        sroc2023And24ChargeInformation,
        sroc2023ChargeInformation,
        srocSupersededChargeInformation,
        alcsChargeInformation
      ]

      // We test that related data is returned in the results. So, we create and link it to the srocChargeInformation
      // ready for testing
    ...
    describe('including those linked to soft-deleted workflow records', () => {
      ...
      it('returns the SROC charge informations that are applicable', async () => {
        ...
        expect(result.chargeInformations).to.have.length(4)
        expect(result.chargeInformations[0].chargeVersionId).to.equal(testRecords[0].chargeVersionId)
        expect(result.chargeInformations[1].chargeVersionId).to.equal(testRecords[1].chargeVersionId)
        expect(result.chargeInformations[2].chargeVersionId).to.equal(testRecords[2].chargeVersionId)
        expect(result.chargeInformations[3].chargeVersionId).to.equal(testRecords[3].chargeVersionId)
      })

      it('returns the licenceIds from SROC charge informations that are applicable', async () => {
        const result = await FetchChargeInformationsService.go(regionId, billingPeriod)
    ...
    it("returns both 'current' and 'superseded' SROC charge informations that are applicable", async () => {
      const result = await FetchChargeInformationsService.go(regionId, billingPeriod)
    ...
    it('includes the related licence and region', async () => {
      ...
      expect(result.chargeInformations[0].licence.licenceRef).to.equal(licenceDefaults.licenceRef)
      expect(result.chargeInformations[0].licence.isWaterUndertaker).to.equal(true)
      expect(result.chargeInformations[0].licence.historicalAreaCode).to.equal(licenceDefaults.regions.historicalAreaCode)
      expect(result.chargeInformations[0].licence.regionalChargeArea).to.equal(licenceDefaults.regions.regionalChargeArea)
      expect(result.chargeInformations[0].licence.region.regionId).to.equal(regionId)
      expect(result.chargeInformations[0].licence.region.chargeRegionId).to.equal(region.chargeRegionId)
    })

    it('includes the related change reason', async () => {
      const result = await FetchChargeInformationsService.go(regionId, billingPeriod)

      expect(result.chargeInformations[0].changeReason.triggersMinimumCharge).to.equal(changeReason.triggersMinimumCharge)
    })
  ...
  describe('when there are no charge informations that should be considered for the next supplementary billing', () => {
    describe("because none of them are linked to a licence flagged 'includeInSrocSupplementaryBilling'", () => {
      beforeEach(async () => {
        ...
        // This creates an SROC charge information linked to a licence. But the licence won't be marked for supplementary
        // billing
        const srocChargeInformation = await ChargeInformationHelper.add()
        testRecords = [srocChargeInformation]
      })
    ...
    describe("because all the applicable charge informations have a 'draft' status", () => {
      beforeEach(async () => {
        billingPeriod = {
          startDate: new Date('2022-04-01'),
          endDate: new Date('2023-03-31')
        ...
        const srocDraftChargeInformation = await ChargeInformationHelper.add({ status: 'draft', licenceId })
        testRecords = [srocDraftChargeInformation]
      })
    ...
    describe("because all of them are for the 'alcs' (presroc) scheme", () => {
      beforeEach(async () => {
        ...
        // This creates an ALCS (presroc) charge information linked to a licence marked for supplementary billing
        const alcsChargeInformation = await ChargeInformationHelper.add({ scheme: 'alcs', licenceId })
        testRecords = [alcsChargeInformation]
      })
    ...
    describe('because none of them have an `invoiceAccountId`', () => {
      ...
      it('returns the licenceId but no applicable charge informations', async () => {
        const result = await FetchChargeInformationsService.go(regionId, billingPeriod)
    ...
    describe('because they all have start dates after the billing period', () => {
      beforeEach(async () => {
        ...
        // This creates an SROC charge information with a start date after the billing period. This will be picked in
    ...
    describe('because the licences flagged for supplementary billing are linked to a different region', () => {
      beforeEach(async () => {
        ...
        // This creates an SROC charge information linked to a licence with an different region than selected
        const otherRegionChargeInformation = await ChargeInformationHelper.add({ licenceId })
        testRecords = [otherRegionChargeInformation]
      })

      it('returns no applicable licenceIds or charge ijnformations', async () => {
        const result = await FetchChargeInformationsService.go(regionId, billingPeriod)
