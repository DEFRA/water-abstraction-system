 ...
 */
export default async function fetchReviewChargeReferenceService(reviewChargeReferenceId) {
    ...
    .modifyGraph('chargeReference', (chargeReferenceBuilder) => {
      chargeReferenceBuilder
        .select([
          ...
          'volume',
          'loss',
          Objection.ref('chargeReferences.additionalCharges:supportedSource.name').castText().as('supportedSourceName'),
          Objection.ref('chargeReferences.additionalCharges:isSupplyPublicWater').castBool().as('waterCompanyCharge')
        ])
