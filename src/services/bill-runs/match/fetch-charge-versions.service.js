...
import Workflow from 'water-abstraction-engine/models/workflow.model.js'
    ...
    .modifyGraph('chargeReferences', (chargeReferencesBuilder) => {
      chargeReferencesBuilder
        .select([
          ...
          'volume',
          'description',
          Objection.ref('chargeReferences.adjustments:aggregate').castFloat().as('aggregate'),
          Objection.ref('chargeReferences.adjustments:s126').castText().as('s126'),
          Objection.ref('chargeReferences.adjustments:s127').castText().as('s127'),
          Objection.ref('chargeReferences.adjustments:s130').castText().as('s130'),
          Objection.ref('chargeReferences.adjustments:winter').castText().as('winter'),
          Objection.ref('chargeReferences.adjustments:charge').castText().as('charge')
        ])
        .whereJsonPath('chargeReferences.adjustments', '$.s127', '=', true)
