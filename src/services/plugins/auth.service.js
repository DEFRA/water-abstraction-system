 ...
 * Used by `AuthPlugin` to authenticate and authorise users
 *
 * We take a user id and look it up in the `idm` schema using `FetchUserAuthDetailsDal`. This gives us a user
...
export default async function authService(_request, session) {
  ...
  const { user, roles, groups } = await FetchUserAuthDetailsDal(userId)
...
function _permission(scope = []) {
  ...
  const users = scope.includes('manage_accounts')
